...
test('an added expense and a set limit survive a page reload', async ({ page }) => {
...
test('clearing all data empties the app, and it stays empty after a reload rather than re-seeding', async ({
  page,
}) => {
  await page.clock.setFixedTime(new Date('2026-06-15T09:00:00'))
  await seedEmptyStorage(page)
